...
class Custom:
    ...
    def LowCmdWrite(self):
        ...
        if self.time_ < self.duration_ :
          ...
          if self.counter_ == 0:
              print(f"Stage 1: Setting to zero posture (0-{self.duration_}s)")
          ...
          if self.counter_ == int(self.duration_ / self.control_dt_):
              print(f"Stage 2: Lifting arms up ({self.duration_}-{self.duration_*3}s)")
          for i,joint in enumerate(self.arm_joints):
          ...
          if self.counter_ == int(self.duration_ * 3 / self.control_dt_):
              print(f"Stage 3: Returning to zero posture ({self.duration_*3}-{self.duration_*6}s)")
          for i,joint in enumerate(self.arm_joints):
          ...
          if self.counter_ == int(self.duration_ * 6 / self.control_dt_):
              print(f"Stage 4: Releasing arm_sdk ({self.duration_*6}-{self.duration_*7}s)")
          for i,joint in enumerate(self.arm_joints):
            ...
            if not self.done:
                print(f"All stages completed! Total time: {self.time_:.2f}s")
            self.done = True
